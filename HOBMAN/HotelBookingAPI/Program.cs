            ...
            { 
                switch (role)
                {
                    case 1:
                        ...
                        Console.WriteLine("Enter City in which you have to book hotel ");

                        if(City=="pune")
                        {
                            LinqToSqlDataContext Linqtosql = new LinqToSqlDataContext();

                           // Linqtosql.BookingDetails();

                            var City = Linqtosql.spSelectHotel2(City);

                            List<HotelDataEntity.Room> 
